...
async def startup_event():
    try:
        ...
        scheduler.add_job(
            svc._ingest_maximo_data,
            "interval", minutes=15, id="maximo_ingest",
        ...
        if sheet_url:
            scheduler.add_job(
                lambda: svc.ingest_cleaning_google_sheet(sheet_url),
                "interval", minutes=30, id="cleaning_ingest",
                max_instances=1, coalesce=True
            )
        scheduler.add_job(
            lambda: celery_app.send_task("optimization.nightly_run"),
            "cron", hour=23, minute=59, id="nightly_opt"
        )
        scheduler.start()
