...
async def generate_daily_briefing(
    ...
    try:
        ...
        return StreamingResponse(
            ...
            headers={"Content-Disposition": f"attachment; filename=daily-briefing-{target_date:%Y-%m-%d}.pdf"},
        )
    except ValueError:
    ...
    try:
        ...
        if format == "csv":
            ...
            return StreamingResponse(
                io.StringIO(content),
                media_type="text/csv",
                headers={"Content-Disposition": f"attachment; filename=assignments-{datetime.now(timezone.utc):%Y-%m-%d}.csv"},
            )
        content = await report_generator.generate_assignments_pdf(assignments)
        return StreamingResponse(
            io.BytesIO(content),
            media_type="application/pdf",
            headers={"Content-Disposition": f"attachment; filename=assignments-{datetime.now(timezone.utc):%Y-%m-%d}.pdf"},
        )
    except ValueError:
        raise HTTPException(status_code=400, detail="Invalid date format. Use YYYY-MM-DD")
    except Exception as exc:
...
async def export_audit_logs(
    ...
    try:
        start_dt = datetime.strptime(start_date, "%Y-%m-%d") if start_date else None
        end_dt = datetime.strptime(end_date, "%Y-%m-%d") if end_date else None
        audit_logs = await audit_service.list_logs(
            start=start_dt,
            end=end_dt,
            user_id=user_id,
            action=action,
        ...
        return StreamingResponse(
            io.StringIO(content),
            media_type="text/csv",
            headers={"Content-Disposition": f"attachment; filename=audit-logs-{datetime.now(timezone.utc):%Y-%m-%d}.csv"},
        )
    except ValueError:
        raise HTTPException(status_code=400, detail="Invalid date format. Use YYYY-MM-DD")
    except Exception as exc:
    ...
    try:
        if format == "pdf":
            ...
            return StreamingResponse(
                io.BytesIO(content),
                media_type="application/pdf",
                headers={"Content-Disposition": f"attachment; filename=fleet-status-{datetime.now(timezone.utc):%Y-%m-%d}.pdf"},
            )
        content = await report_generator.generate_fleet_status_csv()
        return StreamingResponse(
            io.StringIO(content),
            media_type="text/csv",
            headers={"Content-Disposition": f"attachment; filename=fleet-status-{datetime.now(timezone.utc):%Y-%m-%d}.csv"},
        )
    except Exception as exc:
...
async def generate_performance_analysis(
    ...
    try:
        ...
        content = await report_generator.generate_performance_analysis_pdf(start_date, end_date)
        return StreamingResponse(
            io.BytesIO(content),
            media_type="application/pdf",
...
async def generate_compliance_report(
    ...
    try:
        ...
        return StreamingResponse(
            io.BytesIO(content),
            media_type="application/pdf",
            headers={"Content-Disposition": f"attachment; filename=compliance-report-{start_dt:%Y-%m-%d}-to-{end_dt:%Y-%m-%d}.pdf"},
        )
    except ValueError:
        raise HTTPException(status_code=400, detail="Invalid date format. Use YYYY-MM-DD")
    except Exception as exc:
