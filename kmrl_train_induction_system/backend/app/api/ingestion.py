...
async def upload_timeseries(file: UploadFile = File(...)):
    try:
        ...
        for r in records:
            metric = {
                "trainset_id": r.get("trainset_id"), "sensor_type": r.get("sensor_type", "uploaded"),
                "health_score": float(r.get("health_score", 0.0)), "temperature": float(r.get("temperature", 0.0)),
        ...
        if "timestamp" in df.columns:
            ...
            grouped = df.set_index("timestamp").groupby([pd.Grouper(freq="1H"), "trainset_id", "sensor_type"]).mean(numeric_only=True).reset_index()
...
async def ingest_iot_data(background_tasks: BackgroundTasks, current_user: User = Depends(require_role(UserRole.ADMIN))):
    try:
        ...
        return {"message": "IoT data ingestion started", "status": "processing", "source": "iot_sensors"}
    except Exception as e:
...
async def upload_fitness_certificates(file: UploadFile = File(...), _auth=Depends(require_api_key)):
    try:
        svc = DataIngestionService(); content = await file.read(); result = await svc.ingest_fitness_file(content, file.filename)
        return {"status": "ok", "records_processed": result.get("count", 0)}
    except Exception as e:
        logger.error(f"Fitness upload failed: {e}"); raise HTTPException(status_code=400, detail=str(e))

@router.post("/branding/upload")
async def upload_branding_contracts(file: UploadFile = File(...), _auth=Depends(require_api_key)):
    try:
        svc = DataIngestionService(); content = await file.read(); result = await svc.ingest_branding_file(content, file.filename)
        return {"status": "ok", "records_processed": result.get("count", 0)}
    except Exception as e:
        logger.error(f"Branding upload failed: {e}"); raise HTTPException(status_code=400, detail=str(e))

@router.post("/depot/upload")
async def upload_depot_layout(file: UploadFile = File(...), _auth=Depends(require_api_key)):
    try:
        svc = DataIngestionService(); content = await file.read(); result = await svc.ingest_depot_geojson(content)
        return {"status": "ok", "objects": result.get("objects", 0)}
    except Exception as e:
        logger.error(f"Depot layout upload failed: {e}"); raise HTTPException(status_code=400, detail=str(e))

@router.post("/cleaning/google")
async def ingest_cleaning_from_google(sheet_url: str = Form(...), _auth=Depends(require_api_key)):
    try:
        svc = DataIngestionService(); result = await svc.ingest_cleaning_google_sheet(sheet_url)
        return {"status": "ok", "records_processed": result.get("count", 0)}
    except Exception as e:
        logger.error(f"Cleaning sheet ingestion failed: {e}"); raise HTTPException(status_code=400, detail=str(e))
